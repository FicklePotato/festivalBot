...
class Group(object):
    def __init__(self, id, completed_missions):
        self.id = id
...
def dump_groups(json_path, groups):
    with open(json_path, "w") as f:
        json.dump({k: (groups[k].id, groups[k].completed_missions) for k in groups}, f)
...
def load_groups(json_path):
    ...
    with open(json_path, "r") as f:
        groups = json.load(f)
    return {int(k): Group(int(groups[k][ID_LOC]), groups[k][MISSION_LOC]) for k in groups}
